...
pub fn start_vulkan_cube(_state: &mut State, _capability: Uuid) {
    ...
    unsafe {
        ...
        let vert = read_spv(&mut Cursor::new(&include_bytes!("../../../shader/transform.vert.spv")[..])).unwrap();
        let frag = read_spv(&mut Cursor::new(
            &include_bytes!("../../../shader/debug_pattern_world.frag.spv")[..],
        ))
        .unwrap();
        let vert = vk_device
